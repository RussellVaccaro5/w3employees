...
select concat(first_name,' ',last_name) 'Name', salary, (salary*.15) as PF
from employees;
...
select concat(first_name,' ',last_name) 'Names'
from employees;

select trim(first_name)
from employees;

select length(first_name) + length(Last_name) as LengthOfNames
from employees;
...
select employee_id, first_name, last_name, round((salary/12),2) MonthlySalary
from employees;
...
from departments d where e.department_id=d.department_id) department
...
select department_id, department_name, l.location_id, street_address, postal_code, city, state_province, country_name
...
from employees e join departments d using (department_id) join locations l on (d.location_id=l.location_id)
where l.city = 'London';
...
select employee_id, job_title, datediff(end_date,start_date) Days, department_id
from job_history h join jobs j using (job_id)
where department_id=90
group by 1,2;
...
select job_title, first_name, last_name, salary-min_salary Difference
...
select department_name, first_name, last_name, hire_date, salary, (datediff(now(),hire_date))/365 experience
from departments d, employees e
where d.manager_id=e.employee_id
...
select first_name, hire_date, datediff(now(),hire_date)/365 Experience
from employees;
...
select job_id, group_concat(employee_id,' ') EmployeesID
from employees
group by 1;
...
select job_id, LPAD(max_salary, 7, '0') MaxSalary, LPAD(min_salary,7,'0') MinSalary
from jobs;
